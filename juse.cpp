...
void generateJuliaSet(const char* filename, double alpha) {
    vector<Color> data(WIDTH * HEIGHT);

    complex<double> c = 0.7885 * exp(complex<double>(0, alpha));
...
int main() {
    const int numImages = 119; // Количество изображений
    for (int i = 0; i < numImages; i++) {
        double alpha = 2 * PI * i / numImages;
        string filename = "photos/Julia_" + to_string(i) + ".bmp";
        generateJuliaSet(filename.c_str(), alpha);
